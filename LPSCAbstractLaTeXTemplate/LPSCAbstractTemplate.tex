...
% Note that for purposes of revision control, I put each sentence on its own line. 
% This allows the revision control system, git, to highlight each line that's been modified,
% instead of a full paragraph.
% LaTeX doesn't care if there's a (single) newline between each sentence; it
% ignores such whitespace and does the right thing.
...
We have redefined the \verb=\section=, \verb=\subsection=, and \verb=\subsubsection= commands so that text does not start on a new line
...
A monotonically increasing color scheme also does its job whether printed in color or grayscale. See Fig.~\ref{SaturationColors}
...
Using \texttt{bibtex} with the \texttt{inparaenum} option and a new LPSC bibliographystyle approximates the reference-style that has emerged for LPSC abstracts over the years, but there is no hard and fast rule.
